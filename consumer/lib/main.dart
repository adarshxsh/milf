import 'dart:async';
import 'dart:io'; // Added for File reading
...
class WasmJob {
  ...
  WasmJob({required this.fileName, this.sourcePath, this.status = 'Pending'});
}
...
class _ConsumerHomePageState extends State<ConsumerHomePage> {
  ...
  Future<void> _pickAndProcessFiles() async {
    ...
    if (result != null) {
      setState(() {
        _jobs.addAll(
          result.files.map(
            (f) => WasmJob(fileName: f.name, sourcePath: f.path),
          ),
        );
      });
      _processJobs();
  ...
  Future<void> _runSingleJob(WasmJob job) async {
    ...
    Uint8List wasmBytes;
    if (job.sourcePath != null && job.sourcePath!.endsWith('.wasm')) {
      // Read actual file
      try {
        final file = File(job.sourcePath!);
        wasmBytes = await file.readAsBytes();
      } catch (e) {
        setState(() {
          job.status = 'Failed';
          job.output = 'Error reading file: $e';
          _activeJobsCount--;
        });
        return;
      }
    ...
    final result = await _bridge.runWasmTest(wasmBytes, {
      "file": job.fileName,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Execute Job (C/WASM)'),
  ...
  Widget _buildJobsList() {
    if (_jobs.isEmpty) {
      return const Center(
        child: Text(
          'No active jobs. Select .wasm or .c files to begin.',
